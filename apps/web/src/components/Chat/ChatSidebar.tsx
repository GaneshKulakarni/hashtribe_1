...
function ConversationItem({ conversation, isActive, onClick, index }: {
    conversation: Conversation; isActive: boolean; onClick: () => void; index: number;
}) {
    return (
        <motion.button
            ...
            onClick={onClick}
            className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition-all duration-200 text-left group ${isActive ? 'bg-charcoal-800 border border-charcoal-700' : 'hover:bg-charcoal-800/50 border border-transparent'
                }`}
        ...
        >
            <div className="relative flex-shrink-0">
                ...
                {conversation.isOnline && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-charcoal-900" />}
            </div>
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                    <h4 className={`text-sm truncate ${conversation.unreadCount > 0 ? 'text-white font-semibold' : 'text-grey-200 font-medium'}`}>
...
function EmptyState() {
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-grey-500 text-sm text-center">Start a conversation by messaging someone from their profile.</p>
        </motion.div>
    );
}
        ...
        <div className="flex flex-col h-full" id="chat-sidebar">
            <div className="px-4 pt-4 pb-2">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <button className="p-2 rounded-lg text-grey-400 hover:text-white hover:bg-charcoal-800 transition-colors" title="New conversation" id="chat-new-conversation-button">
                ...
                {filteredConversations.length > 0 ? (
                    filteredConversations.map((conversation, index) => (
                        <ConversationItem key={conversation.id} conversation={conversation}
                            ...
                            onClick={() => setActiveConversation(conversation.id)} index={index} />
                    ))
                ) : searchQuery ? (
                    <div className="text-center py-8 px-4"><p className="text-grey-500 text-sm">No results for "{searchQuery}"</p></div>
                ) : (
                    <EmptyState />
                )}
            </div>
            <div className="px-4 py-3 border-t border-charcoal-800">
                <div className="flex items-center space-x-2">
                    ...
                    <p className="text-[11px] text-grey-500 font-mono">{conversations.filter(c => c.isOnline).length} online</p>
                </div>
            </div>
        </div>
    );
