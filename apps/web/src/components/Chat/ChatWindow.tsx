...
function MessageBubble({ message, showAvatar }: { message: ChatMessage; showAvatar: boolean }) {
    return (
        ...
        >
            ...
            {/* Bubble */}
            <div
                className={`max-w-[75%] px-3.5 py-2 rounded-2xl text-sm leading-relaxed ${message.isOwn
                        ? 'bg-white text-black rounded-br-md'
                        : 'bg-charcoal-800 text-grey-100 rounded-bl-md'
                    }`}
            >
                <p className="whitespace-pre-wrap break-words">{message.content}</p>
                <p className={`text-[10px] mt-1 ${message.isOwn ? 'text-grey-500' : 'text-grey-500'}`}>
            ...
            <div className="bg-charcoal-800 rounded-2xl rounded-bl-md px-4 py-3">
                <div className="flex space-x-1">
                    {[0, 1, 2].map(i => (
                        <motion.div
                            key={i}
                            className="w-1.5 h-1.5 bg-grey-500 rounded-full"
                            animate={{ y: [0, -4, 0] }}
                            transition={{
                                duration: 0.6,
                                repeat: Infinity,
        ...
        <div className="flex flex-col h-full" id="chat-window">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-charcoal-800 bg-charcoal-900/50 backdrop-blur-sm">
                <div className="flex items-center space-x-3">
                    <button
                        onClick={onBack}
                        className="p-1.5 rounded-lg text-grey-400 hover:text-white hover:bg-charcoal-800 transition-colors"
                        id="chat-back-button"
                    >
                    ...
                    <div className="relative">
                        <img
                            ...
                            className="w-9 h-9 rounded-full border border-charcoal-700"
                        />
                        {conversation.isOnline && (
                            <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-charcoal-900" />
                        )}
                    </div>
                    <div>
                ...
                <div className="flex items-center space-x-1">
                    <button className="p-2 rounded-lg text-grey-500 hover:text-white hover:bg-charcoal-800 transition-colors" title="Voice call (coming soon)">
                        <Phone className="w-4 h-4" />
                    </button>
                    <button className="p-2 rounded-lg text-grey-500 hover:text-white hover:bg-charcoal-800 transition-colors" title="Video call (coming soon)">
                        <Video className="w-4 h-4" />
                    </button>
                    <button className="p-2 rounded-lg text-grey-500 hover:text-white hover:bg-charcoal-800 transition-colors" title="More options">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 scroll-smooth" id="chat-messages-container">
                ...
                <div className="flex items-center justify-center py-4 mb-2">
                    <div className="flex items-center space-x-2 px-3 py-1.5 bg-charcoal-800/50 rounded-full">
                        ...
                        <p className="text-[10px] text-grey-500 font-mono">
